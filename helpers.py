...
def remove_brand_queries(df, brand_variants):
    ...
    df = df[~df['query'].str.contains('|'.join(brand_variants), case=False, na=False)]
    return df
...
def calculate_click_percentage(df):
    ...
    """
    df['clicks_pct_vs_query'] = df.groupby('query')['clicks'].transform(lambda x: x / x.sum() if x.sum() > 0 else 0)
    return df
...
def merge_with_page_clicks(wip_df, initial_df):
    ...
    wip_df['clicks_pct_vs_page'] = wip_df['clicks_pct_vs_page'].fillna(0)
...
def define_opportunity_levels(wip_df):
    ...
    """
    # Default click threshold for page level is 10%
    page_click_threshold = 0.1
    
    wip_df['comment'] = np.where(
        (wip_df['clicks_pct_vs_query'] >= 0.1) & (wip_df['clicks_pct_vs_page'] >= page_click_threshold),
...
def sort_and_finalize_output(wip_df):
    ...
    wip_df[metric_col_order[:-1]] = wip_df[metric_col_order[:-1]].round(2)
