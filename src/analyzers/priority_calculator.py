...
class PriorityCalculator:
    ...
    def __init__(self, config):
        self.config = config
        self.high_percentile = config.analysis_config.get('priority_percentiles', {}).get('high', 70)
        self.medium_percentile = config.analysis_config.get('priority_percentiles', {}).get('medium', 30)
    ...
    def calculate_priorities(self, data):
        ...
        if priority_mask.sum() > 0:
            ...
            # Assign priorities
            data.loc[priority_mask & (data['traffic_score'] >= high_threshold), 'priority'] = 'High'
            data.loc[priority_mask & (data['traffic_score'] >= medium_threshold) & (data['traffic_score'] < high_threshold), 'priority'] = 'Medium'
            data.loc[priority_mask & (data['traffic_score'] < medium_threshold), 'priority'] = 'Low'
